...
inferl = lbracket
inferr = rbracket
if first_lb > first_rb :
        print("kind1:右括号开头，错误格式！")
        exception2 = True
flag = True
if first_lb == 0 and first_lb > first_rb:
    first_bracket = i
    for d in range(0,first_bracket):
        if a[d] == "(":
            bfr1_lbracket += 1
for inferr in range(0,len(a)):
    for inferl in range(0, len(a)):
        e = a.find(")",inferr,len(a))
        d = a.find("(", inferl, len(a))
        if inferl < inferr and d < e and inferr < d:
           b += 1
           inferl = d
           inferr = e
...
if lbracket - rbracket != 0 and printc == False:
    if lbracket - rbracket > 0:
        print("kind2:该字符串不成对，缺少", c, "个右括号","请检查！")
    else:
        print("kind2:该字符串不成对，缺少", c, "个左括号", "请检查！")
    printc = True
if lbracket == rbracket and bfr1_lbracket == lbracket or flag == True:
    ...
    if printc == False:
        if flag == True:
            if exception2 == True:
                print("kind3:'(其实也是)'字符串中有')('")
            else:
                exception1 = True
                if lbracket == 0 and rbracket == 0:
                    print("kind4:该字符串中无括号")
                    printc = 0
                print("kind5:该字符串内括号成对，且成对数量为",b)
                printc = True
print("This project was programmed by Soulake~")
print("We are looking forward to your creative brainstorm,too!")
print("latest version:8:54,March 9th,2024")
'''
特供：0207版：
目标/可行性分析：给你一串字符串，你要做的是编写一个几乎没有bug的程序，使其能够检查该字符串中括号是否配对
即()是否成对(符合先左再右)，如果成对，输出成对括号数量，否则则输出"括号不配对，请检查!"
